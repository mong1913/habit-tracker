...
class Habit_Tracker:
    ...
    def add_habit(self, habit_name, description, frequency, start_date):
        ...
        try:
            insert_myhabit(
                self.db, habit_name, description, frequency, start_date, 0, 0
            )
    ...
    def fill_history(self, habit_log, period):
        ...
        if period == "day":
            log_dict = {
                datetime.strptime(row[0], "%Y-%m-%d").date(): row[1]
                for row in habit_log
            }
            ...
            log_dict = {
                datetime.strptime(row[0] + "-1", "%G-W%V-%u").date(): row[1]
                for row in habit_log
            }
