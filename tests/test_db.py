...
def test_create_tables(mock_cursor):
    ...
    with mock_cursor() as cur:
        cur.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='myhabit'"
        )
...
def test_get_data_from_myhabit_by_name(init_fake_table):
    ...
    result_none = db.get_data_from_myhabit_by_name(
        init_fake_table, "habit_id", "Reading"
    )
    assert result[0] == 1
...
def test_get_data_from_myhabit_by_period(init_fake_table):
    ...
    result = db.get_data_from_myhabit_by_period(
        init_fake_table, "current_streak", "week"
    )[0]
    assert result == 1
...
def test_get_habit_id_none(init_fake_table, monkeypatch):
    ...
    monkeypatch.setattr(
        db, "get_data_from_myhabit_by_name", lambda arg1, arg2, arg3: []
    )
...
def test_update_myhabit(mock_cursor, init_fake_table):
    with mock_cursor() as cur:
        cur.execute(
            '''SELECT current_streak FROM myhabit WHERE habit_name="Swimming"'''
        )
