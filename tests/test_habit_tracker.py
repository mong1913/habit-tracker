...
def test_add_habit(sut, monkeypatch):
    ...
    assert item_list == [
        ("fake_db.db", "habit_name", "description", "frequency", "start_date", 0, 0)
    ]
...
def test_completion_count(sut, monkeypatch):
    ...
    def fake_get_data_from_tracker(db, columns, habit_name):
        ...
        return [
            ("2025-01-10 12:11", "Done!"),
            ("2025-01-10 18:22", "Done!"),
            ("2025-01-11 17:09", "Skip."),
        ]

    monkeypatch.setattr(
        habit_tracker, "get_data_from_tracker", fake_get_data_from_tracker
    )
...
def test_streak_count_month(sut, monkeypatch):
    ...
    def fake_fill_history(db, habit_log, period):
        ...
        return [
            ...
            (datetime(2025, 2, 1, 0, 0), 3),
            (datetime(2025, 3, 1, 0, 0), 0),
            (datetime(2025, 4, 1, 0, 0), 2),
        ]
